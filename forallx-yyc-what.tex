...
Existem vários tipos de sentenças, vejamos alguns exemplos:

\begin{earg}
\item[\textbf{Sentenças interrogativas}] --  ``Você já está com sono?'', ``Que horas vamos almoçar?''.

\item[\textbf{Sentenças imperativas}] -- ``Sente-se direito'', ``Acorde!''.

\item[\textbf{Sentenças exclamativas}]-- ``Ai, que tragédia!'', ``Nossa!"

\item[\textbf{Sentenças declarativas}] -- ``Choveu em João Pessoa ontem'', ``O asteróide X412 passará perto de Marte em 20 dias''. 
\end{earg}

Uma sentença declarativa diz que algo é assim e assado, e portanto pode ser verdadeira ou falsa.
Naturalmente, ela pode versar sobre cosas que seriam mais consideradas como fatos, \emph{e.g.} ``Kierkegaard era corcunda'' ou ``Kierkegaard gostava de amêndoas'', ou que seriam mais consideradas como opiniões, \emph{e.g.} ``Amêndoas são saborosas''.
Não importa se sabemos, nem mesmo se é possível ou não saber, se a sentença é verdadeira ou falsa para que ela seja declarativa.

Nenhum dos outros tipos de sentenças acima têm (estritamente) essa característica de poder ser verdadeiro ou falso.
Uma pergunta pode ser retórica e dar a entender no contexto algo que pode ser verdadeiro ou falso, mas ela mesma não pode sê-lo.
O mesmo se dá com respeito a uma ordem, ainda que seja aconselhável sentar-se com a coluna ereta, a ordem não será verdadeira ou falsa por causa disso.
Observe que as ordens ou comandos nem sempre são expressos como imperativos.
Por exemplo, a expressão `Você respeitará minha autoridade' \emph{é} ou verdadeira ou falsa, pois você respeitará ou não.
Então, estritamente falando, trata-se de uma sentença declarativa, ainda que consigamos perceber que por trás desta declaração há uma intenção de dar uma ordem.
Uma sentença exclamativa também não pode estritamente ser verdadeira ou falsa, embora frequentemente carregue consigo uma sentença declarativa.
Por exemplo, dizer ``Nossa!'' pode dar a entender algo como ``isso é espantoso''.

Embora possam haver argumentos na linguagem natural contendo sentenças interrogativas, exclamativas ou imperativas, neste livro esses argumentos serão considerados como sendo reformuláveis em argumentos contendo somente sentenças que podem ser verdadeiras ou falsas, isto é, sentenças declarativas. 
A motivação (que não é amplamente aceita) é que sempre que um argumento contém uma sentença não declarativa, esta pode ser transformada em uma sentença declarativa preservando o significado que é pertinente para o argumento em questão.
Sendo assim, a expressão ``sentença'' no sentido lógico neste livro se referirá a sentenças declarativas.
...
Um argumento é, então, inválido, quando existe uma  situação na qual todas as premissas são verdadeiras, mas a conclusão não é.
...
Podemos chamar de \define{nomologicamente válido} um argumento para o qual não há contraexemplos que não violem as leis da natureza.
E podemos chamar de \define{conceitualmente válido} um argumento para o qual não há contraexemplos que não violem as conexões de nossos conceitos.
...
Vamos estudar agora um tipo de validade muito mais estrito que os tipos mencionados acima. Considere este argumento:
	\begin{earg}
		\item[] Chove lá fora.
...
A validade provém dos significados das expressões  `e', `ou', `não' e `se \ldots, então\ldots', que  são denominadas \define{operadores lógicos}. 
A sentença esquemática ``$A$ ou $B$'' é \define{uma forma lógica} da sentença ``Cláudia está no escritório ou no saguão''.\footnote{Como veremos adiante, a seleção de uma ou outra noção de forma lógica é feita por meio de considerações sobre o que é ou não pertinente para capturar a validade de um argumento.}
Assim, esses argumentos são \emph{válidos em virtude de sua forma lógica}, ou simplesmente \emph{válidos formalmente}. 

Podemos chamar de \define{formalmente válido} um argumento para o qual não há contra-exemplos que violem os significados dos operadores lógicos.
