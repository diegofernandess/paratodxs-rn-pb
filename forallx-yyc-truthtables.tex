...
Nos capítulos anteriores, vimos o primeiro aspecto de um sistema lógico: a sua linguagem formal.
O segundo aspecto é uma especificação do conceito de validez.
Assim, um sistema lógico, ou simplesmente, uma \emph{lógica}, consiste em uma linguagem formal juntamente com uma especificação de validez para essa linguagem.
Em seguida procedemos a desenvolver esse segundo aspecto.
A lógica resultante será denominada \emph{lógica proposicional} ou também \emph{lógica verofuncional} (LFV).

Dado um argumento formalizado na linguagem proposicional, para sabermos se ocorre que sempre que suas premissas são verdadeiras, a conclusão também é, devemos saber sob quais circunstâncias as premissas são verdadeiras.
Para tanto, é necessário dar uma \emph{interpretação} para os operadores lógicos: no nosso caso, isso significará dizer em quais casos uma sentença contendo determinado operador como símbolo principal será verdadeira ou não.

%Qualquer sentença da LVF é composta por letras sentenciais, possivelmente combinadas pelos conectivos sentenciais.
Serão feitas as seguintes assunções nesta empreita:
\begin{itemize}
	\item Só existem dois valores de verdade: Verdadeiro e Falso.
	\item O valor de verdade de uma sentença composta depende apenas do valor de verdade das sentenças que a constituem.
\end{itemize}
...
Dadas essas assunções, abaixo são apresentadas interpretações  para os cinco conectivos da linguagem proposicional LP:
...
Isso afeta o modo como devemos entender os nossos esquemas de simbolização.
...
Vamos agora simbolizar este argumento de acordo com o seguinte esquema de simbolização:
	\begin{ekey}
		\item[M] Foi o motorista
		\item[B] Foi a babá
	\end{ekey}
Com este esquema nosso argumento simbolizado na LVF fica:
\begin{earg}
	\item[] $M \eif \enot B$
	\item[] $\enot B$
	\item[\therefore] $M$
...
Vejamos.
De acordo com nosso contraexemplo e esquema de simbolização, `$M$' e `$B$' são ambas falsas, já que no contraexemplo o assassino foi o mordomo, não a babá, nem o motorista.
...
Precisamos de um esquema de simbolização, tal como
\begin{ekey}
	\item[M] Foi o motorista
	\item[B] Foi a babá
\end{ekey}
...
Dado o esquema de simbolização proposta, as mais variadas situações (cenários hipotéticos) que pudermos imaginar para o argumento original vão se ligar a valorações nas quais os valores de verdade das sentenças do argumento simbolizado coincidem com os valores de verdade das sentenças originais dados pelas situações.
...
\end{ekey}
Com este esquema, a simbolização mais natural na LVF da sentença \ref{n:GodParadox} seria:
$$\enot (D \eif M)$$
...
Porém, com o seguinte esquema de simbolização
\begin{ekey}
...
Mudando engenhosamente o esquema de simbolização, você pode usar este argumento para provar a existência de qualquer coisa que você queira.
...
Ou seja, talvez o que queiramos expressar com ela seja melhor dito com outra sentença, tal como
  ...
  \end{earg}
Com o mesmo esquema, a simbolização da sentença \ref{n:GodParadox2} fica:
$$D \eif \enot M$$
...
Considere a seguinte sentença:
	...
	\end{earg}
Considere também o seguinte esquema de simbolização:
\begin{ekey}
	\item[D] É domingo.
	\item [V] Mariana está viva.
	\item[P] Mariana vai à praia.
\end{ekey}
Com este esquema, a simbolização de \ref{n:MariPraia} na LVF fica:
...
Isso, dado nosso esquema de simbolização, significa que no mundo real não acontece jamais de ser domingo e Mariana estar viva, mas ela não ir à praia.
...
Isso, dado nosso esquema de simbolização, significa que em todas as alternativas concebíveis ao mundo real, em todos os cenários hipotéticos imagináveis, não acontece jamais de ser domingo e Mariana estar viva, mas ela não ir à praia.
